...
function UITrainingComplete(StateObjectReference UnitRef)
{
	...
	Alert.EventToTrigger = 'AWCTrainingCompleteAlert_LW';
...
simulated function TrainingCompleteCB(EUIAction eAction, UIAlert AlertData, optional bool bInstanta = false)
{
	...
	if (!HQPres.m_kAvengerHUD.Movie.Stack.HasInstanceOf(class'UIArmory_AWCExpandedPromotion_LW')) // If we are already in the promotion screen, just close this popup
	{
		if (eAction == eUIAction_Accept)
		{
			GoToArmoryAWCPromotion(AlertData.UnitInfo.UnitRef, true);
		}
		else
		{
