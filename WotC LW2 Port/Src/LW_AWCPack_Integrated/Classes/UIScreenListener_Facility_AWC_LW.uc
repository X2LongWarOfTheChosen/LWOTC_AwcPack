...
event OnInit(UIScreen Screen)
{
	...
	local UIFacility_AdvancedWarfareCenter ParentScreen;

	//default is no dropdown
	QueuedDropDown = -1;

	ParentScreen = UIFacility_AdvancedWarfareCenter(Screen);
...
{
	UIFacility_AdvancedWarfareCenter(Screen).m_kStaffSlotContainer.Show();
}
// This event is triggered after a screen loses focus
event OnLoseFocus(UIScreen Screen)
{
	UIFacility_AdvancedWarfareCenter(Screen).m_kStaffSlotContainer.Hide();
}
...
defaultproperties
{
	...
	ScreenClass = UIFacility_AdvancedWarfareCenter;
}